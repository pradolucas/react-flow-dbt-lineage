...
const iconStyle = { marginRight: "10px", flexShrink: 0 };
...
const expanderTextStyle = { fontSize: "12px", fontWeight: "500" };
...
const DataTypeIcon = ({ type }) => {
  const dataTypeIconMap = {
    ...
    binary: {
      ...
      icon: (
        ...
        >
          <path d="M6 10v4" />
          <path d="M10.5 10h1.5c.8 0 1.5.7 1.5 1.5v1c0 .8-.7 1.5-1.5 1.5h-1.5" />
          <path d="M16 14v-4h2" />
          <path d="M18 10v4" />
        </svg>
      ),
  ...
  return (
    <div title={type}>
      {foundIcon ? foundIcon.icon : dataTypeIconMap.default.icon}
    </div>
  );
};
      ...
      <div style={columnContainerStyle}>
        {columnsToShow.map((column, index) => {
          ...
          const dynamicColumnStyle = {
            ...columnBaseStyle,
            ...(index === columnsToShow.length - 1 && !hasManyColumns
              ? { borderBottom: "none" }
              : {}),
